...
class CustomerMembershipsDB {
    ...
    public static function add_foreign_keys() {
        ...
        $constraints = [
            ...
            // FK to app_customer_membership_levels (current level)
            [
                'name' => 'fk_membership_level',
                'sql' => "ALTER TABLE {$table_name}
                         ADD CONSTRAINT fk_membership_level
                         FOREIGN KEY (level_id)
                         REFERENCES {$wpdb->prefix}app_customer_membership_levels(id)
                         ON DELETE RESTRICT"
            ],
            // FK to app_customer_membership_levels (upgrade target level, nullable)
            [
                'name' => 'fk_membership_upgrade_level',
                'sql' => "ALTER TABLE {$table_name}
                         ADD CONSTRAINT fk_membership_upgrade_level
                         FOREIGN KEY (upgrade_to_level_id)
                         REFERENCES {$wpdb->prefix}app_customer_membership_levels(id)
                         ON DELETE SET NULL"
            ]
        ];
        ...
        foreach ($constraints as $constraint) {
            ...
            if ($result === false) {
                error_log("[CustomerMembershipsDB] Failed to add FK {$constraint['name']}: " . $wpdb->last_error);
            }
        }
    }
}
