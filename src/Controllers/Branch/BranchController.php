 ...
 *
 * Changelog:
 * 1.3.3 - 2025-11-02 (TODO-2190 Fix Inspector Assignment)
 * - CRITICAL FIX: Removed auto-update agency_id/division_id on location change
 * - Changed: If provinsi/regency changes and branch has inspector, clear all three fields
 * - Reason: Inspector assignment is location-specific, invalid after location change
 * - User must manually re-assign inspector after changing branch location
 * - agency_id and division_id ONLY set via "assign inspector" feature
 *
 * 1.3.2 - 2025-11-02 (TODO-2190 Fix Agency Assignment)
 * - Removed: agency_id and division_id assignment from create flow
 * - Reason: Agency/Division untuk assign inspector, bukan create branch
 * - Changed: Branch created without agency/division (will be assigned later)
 * - Fixed: Error "Agency not found" saat create branch
 * - Schema: agency_id made nullable
 *
 * 1.3.1 - 2025-11-02 (TODO-2190 Auto-Generate Password)
 * - Removed: Password validation (password + password_confirm)
 * - Changed: Password auto-generated by WordPress/AutoEntityCreator
 * - Changed: wp_insert_user() without user_pass = auto random password
 * - Benefit: Simpler UX, secure random password, sent via email
 * - Still: Force password reset on first login (default_password_nag)
 *
 * 1.3.0 - 2025-11-02 (TODO-2190 Default Type Cabang)
 * - Changed: Type hardcoded sebagai 'cabang' (default)
 * - Removed: Type field dari form (tidak perlu dipilih user)
 * - Removed: validateBranchTypeCreate() call (tidak diperlukan lagi)
 * - Reason: Cabang Pusat sudah auto-created via AutoEntityCreator
 * - Manual create branch hanya untuk tambah cabang baru
 *
 * 1.1.3 - 2025-11-02 (TODO-2190 Fix - Complete Role Assignment)
 * - FIXED: Added missing 'customer' role to branch user creation
 * - User dari branch sekarang mendapat 3 role:
 ...
 * - AutoEntityCreator already working for auto-create employee record
 *
 * 1.1.2 - 2025-11-02 (TODO-2190 Fix - Security: Nonce Mismatch)
 * - SECURITY FIX: Changed form nonce from wpapp_panel_nonce to wp_customer_nonce
 * - Updated create-branch-form.php and edit-branch-form.php nonce generation
 * - handle_save_branch() removed redundant nonce check (delegate to internal methods)
 * - Maintains CSRF protection with proper nonce validation in update() and store()
 * - Fixes 403 error when saving form while preserving security
 *
 * 1.1.1 - 2025-11-02 (TODO-2190 Fix)
 * - Fixed: handle_get_branch_form() nonce parameter from 'nonce' to '_ajax_nonce'
 * - Fixes 403 Forbidden error when loading modal forms
 * - Nonce now passed via URL query parameter instead of POST
 *
 * 1.1.0 - 2025-11-02 (TODO-2190)
 * - Added: handle_get_branch_form() - Load create/edit form for modal
 * - Added: handle_save_branch() - Wrapper for create/update operations
 * - Integrated: wp-app-core centralized modal system
 * - Registered: AJAX actions for modal integration
 *
 * 1.0.0 - 2024-12-10
     ...
     */
        ...
        public function store() {
            try {
                ...
                // Note: Type validation tidak diperlukan lagi karena type sudah hardcoded sebagai 'cabang'
                // Cabang pusat sudah auto-created via AutoEntityCreator saat customer registration
